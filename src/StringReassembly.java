...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // uses the substring method to go through the strings and concatenate

        String combinationConcatenationOfStrOneAndStrTwo = (str1
                .substring(0, str1.length() - overlap).concat(str2));

        return combinationConcatenationOfStrOneAndStrTwo;
        // returns the value.
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // when the size is 0 it just adds the str val
        if (strSet.size() == 0) {
            strSet.add(str);
        } else {
            // when it isnt 0, it goes through ,finds the string
            String valueForTheSet = strSet.removeAny();
            if (!valueForTheSet.contains(str)) {
                // check if it doesnt contain
                addToSetAvoidingSubstrings(strSet, str);
            }
            if (!str.contains(valueForTheSet)) {
                strSet.add(valueForTheSet);
            }
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> txtFileInputtedLines = new Set1L<String>();
        // new set of the string

        while (!input.atEOS()) {
            // keeps adding the variable
            addToSetAvoidingSubstrings(txtFileInputtedLines,
                    (input.nextLine()));
        }

        return txtFileInputtedLines;
        // returns it
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // intiailize the values.
        int x = 0;
        int lengthOfText = text.length();
        int lengthOfTextMinusOne = lengthOfText - 1;

        /*
         * keep on printing out the value and when it is "~" it is replaced w/
         * new lines
         */

        while ((lengthOfTextMinusOne) > x) {
            if (text.charAt(x) == '~') {
                out.println();
            } else {
